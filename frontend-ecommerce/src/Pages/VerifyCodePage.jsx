...
const VerifyCodePage = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      await axios.post(config.getApiUrl("/api/auth/verify-code"), { email, code });
      setMessage("Verification successful! Logging you in...");

      // Step 2: Auto-login using email & password
      const loginRes = await axios.post(config.getApiUrl("/api/auth/login"), {
        email,
        password,
      });
  ...
  const handleResend = async () => {
    setLoading(true);
    try {
      await axios.post(config.getApiUrl("/api/auth/resend-code"), { email });
